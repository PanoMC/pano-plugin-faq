<tr>
    ...
    <th scope="row" class="align-middle text-center" style="width: 60px;">
        <div class="dropdown position-static">
            <button type="button" class="btn btn-link" data-bs-toggle="dropdown" aria-expanded="false" aria-label={$_('actions')}>
                <i class="fas fa-ellipsis-v"></i>
            </button>
            <ul class="dropdown-menu animate__animated animate__fadeIn">
                <li>
                    <button type="button" class="dropdown-item" on:click={() => onEdit(faq)}>
                        <i class="fas fa-pen me-2"></i> {$_('faq.edit_faq')}
                    </button>
                </li>
                <li>
                    <button type="button" class="dropdown-item text-danger" on:click={() => onDelete(faq)}>
    ...
    <td class="align-middle">
        <div class="text-truncate" style="max-width: 300px;">
            <button class="btn btn-link p-0 text-start text-decoration-none fw-bold w-100 text-truncate" 
                    on:click={() => onEdit(faq)}>
                {faq.question}
            </button>
        </div>
